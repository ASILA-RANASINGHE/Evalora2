...
function ShortNoteCard({ note }: { note: ShortNote }) {
  return (
    <Link href={`/protected/student/short-notes/${note.id}`}>
...
export default async function SubjectShortNotesPage({ params }: { params: Promise<{ subject: string }> }) {
  const { subject } = await params;
  const subjectName = subject
    .split("-")
    .map((w) => w.charAt(0).toUpperCase() + w.slice(1))
    .join(" ");
  const { adminContent, teacherContent } = await getShortNotesBySubject(subjectName);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-2">
        <Link href="/protected/student/short-notes" className="text-muted-foreground hover:text-foreground">
          <ChevronLeft className="h-5 w-5" />
        </Link>
        <h2 className="text-3xl font-bold tracking-tight">{subjectName} Short Notes</h2>
      </div>

      {!hasContent ? (
        <div className="text-center py-20">
          <p className="text-muted-foreground text-lg">No short notes available yet.</p>
