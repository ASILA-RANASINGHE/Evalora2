...
import type { ChildCard, ParentUpcomingMilestone } from "@/lib/actions/analytics";
...
export function ParentDashboardClient({ parentName, children, upcomingMilestones }: Props) {
  ...
  if (!selected) {
    return (
      <div className="space-y-6">
        <div>
          <h2 className="font-space-grotesk text-2xl font-bold">Good morning, {parentName}</h2>
          <p className="text-muted-foreground mt-1">No linked students found. Contact your school to link your children.</p>
        </div>
      </div>
    );
  }
  ...
  // Dynamic date range for "Weekly Report"
  const now = new Date();
  const weekAgo = new Date(now.getTime() - 7 * 24 * 60 * 60 * 1000);
  const dateRangeLabel = `${weekAgo.toLocaleDateString("en-US", { month: "short", day: "numeric" })} — ${now.toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}`;
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        <h2 className="font-space-grotesk text-2xl font-bold">Good morning, {parentName}</h2>
        <p className="text-muted-foreground mt-1">Here&apos;s how {selected.name.split(" ")[0]} is doing</p>
      </div>
            ...
            <CardContent className="space-y-4">
              <div className="p-4 rounded-lg bg-gradient-to-br from-purple-50 to-indigo-50 dark:from-purple-900/10 dark:to-indigo-900/10 border border-purple-200 dark:border-purple-800">
                ...
                <p className="text-sm text-muted-foreground">
                  Your weekly performance summary is ready for review.{" "}
                  {children.length === 1
                    ? `${children[0].name.split(" ")[0]} has been active this week.`
                    : `Your ${children.length} children have been active this week with progress tracked across their subjects.`}
                </p>
              </div>
          ...
          <Card className="border-purple-200 dark:border-purple-800 bg-purple-100/80 dark:bg-purple-900/40 backdrop-blur-md shadow-xl shadow-purple-900/5 ring-1 ring-purple-500/10 h-full">
            ...
            <CardContent className="space-y-3">
              {upcomingMilestones.length === 0 ? (
                <p className="text-sm text-muted-foreground text-center py-4">No upcoming milestones. All caught up!</p>
              ) : (
